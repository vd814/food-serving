...
var userDatatable=$('#requestDataTable').DataTable( {
	...
	"bSort":true,
	"ajax": {
		"url": "getAllRequestWithDataTable",
		"data": function ( data ) {
		}
	},
	"aoColumnDefs": [
...
function assignRequest(rID) {
	$("#requestID").val(rID);
	$("#assignRequest").trigger('click');
}
...
function cancelRequest(requestID){
	$("#requestIdForCancelRequest").val(requestID);
...
function requestCancelReason(requestID) {
	...
	var role = '';
	if (requestCancel[0][1].role == userRole) {
		role = 'Self';
	}else if (requestCancel[0][1].role == user) {
		role = 'Public User';
