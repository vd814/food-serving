...
var userDatatable=$('#requestDeclineDataTable').DataTable( {
	...
	"bSort":true,
	"ajax": {
		"url": "getAllVendorRequestDeclineWithDatatable",
		"data": function ( data ) {
		}
	},
	"aoColumnDefs": [
...
function editRequestDecline(requestDeclineID) {
	var requestDecline = getRequestDeclineByID(requestDeclineID);
	$("#requestDeclineID").val(requestDecline.id);
	$("#declineReason").val(requestDecline.reason);
	$("#forRequestDeclineReason").trigger('click');
}

function getRequestDeclineByID(requestDeclineID) {
	var data = '';
	$.ajax({
		url:'getRequestDeclineByID',
		type:'post',
		dataType:'json',
		data:{_csrf:token,requestDeclineID:requestDeclineID},
		async:false,
		success:function(res){
			data = res;
		}
